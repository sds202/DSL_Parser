...
namespace winrt::DSL_parser_winui3::implementation
{
    ...
    void MainWindow::OnStartChatClick(Windows::Foundation::IInspectable const&, RoutedEventArgs const&)
    {
        ...
        m_userId = userId;
        WelcomeInterface().Visibility(Visibility::Collapsed);
        ChatInterface().Visibility(Visibility::Visible);

        // Automatically send "你好" (Unicode escaped to avoid encoding issues)
        // We do NOT append this message to the UI, so it appears as if the server initiated the conversation.
        hstring initialMessage = L"\u4F60\u597D";
        SendMessageAsync(initialMessage);
    }
