...
def mock_chat_completion():
    ...
    entity_name_in_prompt = ""
    # 从 system prompt 中提取 entity_name_in_prompt

    if system_prompt:
        # 1️⃣ 用正则提取 “实体：[ ... ]” 中的内容
        match = re.search(r"实体：\[(.*?)\]", system_prompt, re.S)

        if match:
            entity_block = match.group(1).strip()
            # entity_block 示例: "订单号 手机号 用户名"

            # 2️⃣ 按空格拆分
            entities = [e for e in entity_block.split() if e]

            # 3️⃣ 只取第二个实体
            if len(entities) >= 2:
                entity_name_in_prompt = entities[1]
    ...
    if any(w in user_input for w in ["你好", "hello", "hi", "您好"]):
        ...
        if order_match:
            intent = "QUERY_ORDER"
            # 提取出的纯数字 ID
            entities[entity_name_in_prompt] = order_match.group(1)
            print(f"[INFO] 提取实体: {entity_name_in_prompt} = {entities[entity_name_in_prompt]}")
