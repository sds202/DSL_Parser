...
def init_dsl_v1(client):
    """保证测试开始时 DSL 是正常的"""
    resp = client.post_admin("./DSL_script/test2.dsl")
...
def test_03_less_valid_dsl_file(client):
    ...
    assert "missing" in r2.text and ")" in r2.text
...
def test_04_invalid_dsl_file_unknown_call(client):
    ...
    assert r2.status_code == 500
    assert "line 2" in r2.text and "Undefined" in r2.text
